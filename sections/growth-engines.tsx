          ...
          {GROWTH_ENGINES.engines.map((engine, i) => {
            ...
            return (
              <StaggerItem key={engine.title}>
                <SpotlightCard className="h-full p-9 md:p-11" glow="rgba(31,122,83,0.10)">
                  <div className="flex items-start justify-between">
                    <span className="grid size-14 place-items-center rounded-2xl bg-gradient-to-br from-forest-600 to-forest-800 text-white shadow-[0_10px_30px_-10px_rgba(16,85,34,0.6)] transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3">
                      <Icon className="size-6" aria-hidden="true" />
                    </span>
                    <span className="font-labels rounded-full border border-gold-600/25 bg-gold-500/10 px-3.5 py-1.5 text-[12px] font-semibold tracking-[0.14em] text-gold-600 uppercase">
                      {engine.eyebrow}
                    </span>
                  </div>
                  ...
                  <a
                    href="#cta"
                    className="group/link font-labels mt-9 inline-flex items-center gap-2 text-[15px] font-semibold text-forest-700 transition-colors hover:text-forest-500"
                  >
                    {engine.cta}
